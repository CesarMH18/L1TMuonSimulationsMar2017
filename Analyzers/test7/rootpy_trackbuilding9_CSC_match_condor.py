...
def is_RPC_hit(hit):
  is_rpc = (hit.type == kRPC) and (hit.station == 3 or hit.station == 4)
  return  is_emtf_legit_hit(hit) and is_rpc
...
class Histos_matching(object):
  def __init__(self):
    ...
    hnames = ["CSC_hits_total","CSC_hits_iRPC_chamber","iRPC_hits_total","iRPC_hits_iRPC_chamber"]
    for hname in hnames:
      self.histos[hname] = Hist(15, -0.5, 14.5, name=hname, title=hname, type='F')
      self.names_histos.append(hname)

    hname = "Part_iRPC_chamber"
    self.histos[hname] = Hist(2, -0.5, 1.5, name=hname, title="Number of muons in iRPC chambers / Event", type='F')
    self.names_histos.append(hname)

    hnames = ["CSC_RPC_dx", "CSC_RPC_dy", "Part_CSC_dx", "Part_CSC_dy", "Part_RPC_dx", "Part_RPC_dy"]
    for hname in hnames:
      self.histos[hname] = TH1F(hname, hname, 401, -200., 200.)
      self.names_histos.append(hname)

    hname = "CSC_RPC_dR"
    self.histos[hname] = TH1F(hname,hname, 101, 0., 100.)
    self.names_histos.append(hname)

    hnames = ["Part_CSC_dR", "Part_RPC_dR"]
    for hname in hnames:
      self.histos[hname] = TH1F(hname, hname, 101, 0., 100.)
      self.names_histos.append(hname)

    hnames = ["CSC_RPC_dxy", "Part_CSC_dxy", "Part_RPC_dxy"]
    for hname in hnames:
      self.histos[hname] = TH2F(hname, hname, 401, -200., 200., 401, -200, 200)
      self.names_histos.append(hname)
  ...
  def write(self, filename='histos_matching_rates.root'):
...
def CSCmatch_allRPC(hit, RPChits):
  '''Matching CSC hit using RPC endcap after propagating from point(0,0,0)
  return matrix isMatchME'''
  xslope = hit_x(hit)/hit.sim_z
  yslope = hit_y(hit)/hit.sim_z
  isMatchME = np.zeros((25,25), dtype=bool)
  RPChit_match_time = np.full((25,25),26,dtype=int)
  for RPChit in RPChits:
      if (RPChit.sim_z * hit.sim_z<0) or hit.station!=RPChit.station:
        continue
      if hit.ring==2 and RPChit.ring==1:
        continue
      ...
      z = RPChit.sim_z
      #distance
      Dx = abs(hit_x(RPChit) - x)
      Dy = abs(hit_y(RPChit) - y)
      for i in range(0,25):
        for j in range(0,25):
            if (Dx < i+1 and Dy < j+1):
              isMatchME[i,j]=True
              RPChit_match_time[i,j]=RPChit.time
  return isMatchME, RPChit_match_time
...
class PatternRecognition(object):
  ...
  def nearestRPCtoCSC(self, hit, RPC_hits):
    ...
    yslope = hit_y(hit)/hit.sim_z
    Dist_min = 100.
    Dx_min, Dy_min = 100., 100.
    for RPChit in RPC_hits:
      if (RPChit.sim_z * hit.sim_z<0) or hit.station!=RPChit.station or hit.ring!=RPChit.ring:
        continue
      dz = RPChit.sim_z - hit.sim_z
      dx = dz * xslope
      dy = dz * yslope
  ...
  def partHitmatch(self, part, hit):
    ...
    #part_x = hit.sim_z*math.tan(np.rad2deg(np.arctan2(1.0, np.sinh(part.eta))))*math.cos(part.phi)
    #part_y = hit.sim_z*math.tan(np.rad2deg(np.arctan2(1.0, np.sinh(part.eta))))*math.sin(part.phi)
    dx = part_x - hit_x(hit)
    dy = part_y - hit_y(hit)
    dR = math.sqrt(dx*dx + dy*dy)
    return dx,dy,dR

  def CSCmatch(self, hit, RPChits):
    '''Matching CSC hit using iRPC after propagating from point(0,0,0)
    return matrix isMatchME and matched RPC hit time'''
    xslope = hit_x(hit)/hit.sim_z
    yslope = hit_y(hit)/hit.sim_z
    isMatchME = np.zeros((25,25), dtype=bool)
    RPChit_match_time = np.full((25,25),26,dtype=int)
  ...
  def run(self, hits, part, cut_match_dR=100):
    ...
    csc_counter_total=0
    irpc_counter_total=0
    csc_counter=0
    irpc_counter=0


    #variables for Matching
    DX_rpc,DY_rpc,DR_rpc=200.,200.,200.
    DX_csc,DY_csc,DR_csc=200.,200.,200.
    iRPC_hits=[]
    CSC_hits=[]
    ...
    for ihit, hit in enumerate(legit_hits):
      ...
      #Counting number of events and CSC-iRPC hits
      if ((hit.station == 3 or hit.station == 4) and hit.ring == 1):
        if hit.type==kRPC:
          irpc_counter_total=irpc_counter_total+1
          if part.eta>1.8:
            irpc_counter=irpc_counter+1
            dx,dy,dr=self.partHitmatch(part,hit)
            iRPC_hits.append(hit)
            if dr<DR_rpc:
              DX_rpc,DY_rpc,DR_rpc=dx,dy,dr
        elif hit.type==kCSC:
          csc_counter_total=csc_counter_total+1
          if part.eta>1.8:
            csc_counter=csc_counter+1
            dx,dy,dr=self.partHitmatch(part,hit)
            CSC_hits.append(hit)
            if dr<DR_csc:
              DX_csc,DY_csc,DR_csc=dx,dy,dr

    #matchin CSC to iRPC
    if part.eta>1.8:
      self.histos.histos['Part_iRPC_chamber'].Fill(1)
      for cschit in CSC_hits:
        if (cschit.station==3 and cschit.sim_eta>1.83) or (cschit.station==4 and cschit.sim_eta>1.92):
          dx,dy,dr=self.nearestRPCtoCSC(hit,iRPC_hits)
          self.histos.histos['CSC_RPC_dR'].Fill(dr)
          self.histos.histos['CSC_RPC_dx'].Fill(dx)
          self.histos.histos['CSC_RPC_dy'].Fill(dy)
          self.histos.histos['CSC_RPC_dxy'].Fill(dx,dy)


    #Saving in histos
    self.histos.histos['CSC_hits_total'].Fill(csc_counter_total)
    self.histos.histos['iRPC_hits_total'].Fill(irpc_counter_total)
    self.histos.histos['CSC_hits_iRPC_chamber'].Fill(csc_counter)
    self.histos.histos['iRPC_hits_iRPC_chamber'].Fill(irpc_counter)

    if part.eta>1.8 and csc_counter>0:
      self.histos.histos['Part_CSC_dx'].Fill(DX_csc)
      self.histos.histos['Part_CSC_dy'].Fill(DY_csc)
      self.histos.histos['Part_CSC_dxy'].Fill(DX_csc,DY_csc)
      self.histos.histos['Part_CSC_dR'].Fill(DR_csc)

    if part.eta>1.8 and irpc_counter>0:
      self.histos.histos['Part_RPC_dx'].Fill(DX_rpc)
      self.histos.histos['Part_RPC_dy'].Fill(DY_rpc)
      self.histos.histos['Part_RPC_dxy'].Fill(DX_rpc,DY_rpc)
      self.histos.histos['Part_RPC_dR'].Fill(DR_rpc)
      ...
      for sector in (1, 2, 3, 4, 5, 6):
        ...
        def match_csc(hit):
          ...
          if hit.type == kCSC and ((hit.station == 3 or hit.station == 4) and hit.ring == 1):
            if (hit.station==3 and hit.sim_eta>1.83) or (hit.station==4 and hit.sim_eta>1.92):
              dx,dy,dR = self.nearestRPCtoCSC(hit,iRPC_hits)
              if dR>cut_match_dR:
                match=False
                self.histos.histos['CSC_no_match'].Fill(1)
          return match


        sector_hits = list(filter(match_csc, sector_hits))
  ...
  def run(self, omtf_input=False, run2_input=False, pileup=200, match_all_endcap=False, cut_match_dR=100):
    ...
    if use_condor:
      if match_all_endcap==True:
        ...
        outfile = 'histos_tbb_files_%i-%i_CSCmatch_iRPC_cutDR_%i.root' % (file_first,file_last, cut_match_dR)
  ...
  def run(self, omtf_input=False, run2_input=False, match_all_endcap=False, cut_match_dR=100):
    ...
    if use_condor:
      if match_all_endcap==True:
        ...
        outfile = 'histos_tbc_files_%i-%i_CSCmatch_iRPC_cutDR_%i.root' % (file_first,file_last, cut_match_dR)
        # Save histograms Matching
        recog1.histos.write(outfile.replace('tbc','matching'))
