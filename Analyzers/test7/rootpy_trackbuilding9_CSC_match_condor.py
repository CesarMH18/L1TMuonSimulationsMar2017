...
from ROOT import gROOT, TH1 , TH2I, TH2F, TH1F
gROOT.SetBatch(True)
...
class Histos_matching(object):
  def __init__(self):
    self.histos_hits =  np.empty((2,2,6), dtype=np.object)
    self.histos_hits_muon =  np.empty((2,2,6), dtype=np.object)
    self.histos_station =  np.empty((3,), dtype=np.object)
    self.histos_station_muon =  np.empty((3,), dtype=np.object)
    self.histos = {}
    self.names_histos = []
    ...
    for endcap in [-1,+1]:
      for station in [3,4]:
        for sector in [1,2,3,4,5,6]:
          endcap_ = endcap+1 if endcap==-1 else endcap
          station_= station-3
          sector_ = sector-1

          hname='CSC_hits_REplus_stationSTA_sectorSEC' if endcap==1 else 'CSC_hits_REminus_stationSTA_sectorSEC'
          hname=hname.replace("STA",str(station)).replace("SEC",str(sector))
          self.histos_hits[endcap_,station_,sector_] = TH1F(hname, hname+' per Event', 11,-.5, 10.5)

          hname=hname.replace("hits","hits_muon")
          self.histos_hits_muon[endcap_,station_,sector_] = TH1F(hname, hname+' per Event', 11,-.5, 10.5)


    for i,hname in enumerate(['CSC_hits_total','CSC_hits_RE3','CSC_hits_RE4'],0):
      self.histos_station[i] = TH1F(hname, hname+' per Event', 11,-.5, 10.5)
      hname=hname+"_muon"
      self.histos_station_muon[i] = TH1F(hname, hname+' per Event', 11,-.5, 10.5)


    hnames = ["muon_CSC_RE31_dR", "muon_CSC_RE31_dx", "muon_CSC_RE31_dy",
              "muon_CSC_RE41_dR", "muon_CSC_RE41_dx", "muon_CSC_RE41_dy",
              "CSC_iRPC_dR", "CSC_iRPC_dx", "CSC_iRPC_dy"
              ]
    for hname in hnames:
      self.histos[hname] = TH1F(hname, hname, 100, 0.00001, 100.00001)
      self.names_histos.append(hname)

    hnames = ["CSC_iRPC_dxy", "muon_CSC_RE31_dxy","muon_CSC_RE41_dxy"]
    for hname in hnames:
      self.histos[hname] = TH2F(hname, hname, 200, -99.999, 100.001,200, -99.999, 100.001)
      self.names_histos.append(hname)

    hnames = ["CSC_no_matched","CSC_matched"]
    for hname in hnames:
      self.histos[hname] = TH1F(hname, hname, 3, 0., 2.)
      self.names_histos.append(hname)

  def write(self, filename='histos_matching.root'):
    with root_open(filename, 'recreate') as f:
      for i in [0,1,2]:
        self.histos_station[i].Write()
        self.histos_station_muon[i].Write()
      for name in self.names_histos:
        h = self.histos[name]
        h.Write()
      for endcap in [0,1]:
        for station in [0,1]:
          for sector in [0,1,2,3,4,5]:
              self.histos_hits[endcap,station,sector].Write()
              self.histos_hits_muon[endcap,station,sector].Write()

def hit_x(hit):
  x = hit.sim_r * math.cos(math.radians(hit.sim_phi))
  return x
def hit_y(hit):
  y = hit.sim_r * math.sin(math.radians(hit.sim_phi))
  return y
...
class PatternRecognition(object):
  ...
  def nearestRPCtoCSC(self, hit, rpc_hits):
    xslope = hit_x(hit)/hit.sim_z
    yslope = hit_y(hit)/hit.sim_z
    dist_min = 1000.
    dx_min, dy_min = 1000., 1000.
    for rpchit in rpc_hits:
      if (rpchit.endcap * hit.endcap<0) or hit.station!=rpchit.station or hit.ring!=rpchit.ring:
        continue
      dz = rpchit.sim_z - hit.sim_z
      dx = dz * xslope
      dy = dz * yslope
      #new extrapolated point
      x = hit_x(hit) + dx
      y = hit_y(hit) + dy
      #distance
      Dx = x - hit_x(rpchit)
      Dy = y - hit_y(rpchit)
      D_aux = math.sqrt(Dx*Dx + Dy*Dy)
      if (D_aux<dist_min):
        dx_min, dy_min = Dx, Dy
        dist_min = D_aux
    return dx_min,dy_min,dist_min

  def muonCSCmatch(self, part, csc_hits):
    dr_min_re3 = 1000.
    dx_min_re3, dy_min_re3 = 1000., 1000.
    dr_min_re4 = 1000.
    dx_min_re4, dy_min_re4 = 1000., 1000.
    for hit in csc_hits:
      part_x = abs(hit.sim_z-part.vz)*math.tan(part.theta)*math.cos(part.phi) + part.vx
      part_y = abs(hit.sim_z-part.vz)*math.tan(part.theta)*math.sin(part.phi) + part.vy
      dx = part_x - hit_x(hit)
      dy = part_y - hit_y(hit)
      dR = math.sqrt(dx*dx + dy*dy)
      if hit.station==3 and dR<dr_min_re3:
        dx_min_re3, dy_min_re3, dr_min_re3 = dx, dy, dR
      elif hit.station==4 and dR<dr_min_re4:
        dx_min_re4, dy_min_re4, dr_min_re4 = dx, dy, dR
    self.histos.histos['muon_CSC_RE31_dx'].Fill(dx_min_re3)
    self.histos.histos['muon_CSC_RE31_dy'].Fill(dy_min_re3)
    self.histos.histos['muon_CSC_RE31_dxy'].Fill(dx_min_re3,dy_min_re3)
    self.histos.histos['muon_CSC_RE31_dR'].Fill(dr_min_re3)
    self.histos.histos['muon_CSC_RE41_dx'].Fill(dx_min_re4)
    self.histos.histos['muon_CSC_RE41_dy'].Fill(dy_min_re4)
    self.histos.histos['muon_CSC_RE41_dxy'].Fill(dx_min_re4,dy_min_re4)
    self.histos.histos['muon_CSC_RE41_dR'].Fill(dr_min_re4)
    return True


  def run(self, hits, part, cut_match_dR=100):
    roads = []
    ...
    legit_hits = list(filter(is_emtf_legit_hit, hits))
    irpc_hits  = list(filter(is_irpc_hit, legit_hits))

    def is_csc_hit(hit):
      is_csc = (hit.type == kCSC) and ((hit.station == 3 or hit.station == 4) and \
                hit.ring == 1) and abs(part.eta)>1.8 and abs(part.eta)<2.5
      return is_csc

    csc_hits = list(filter(is_csc_hit, legit_hits))
    self.muonCSCmatch(part, csc_hits)

    #counting csc hits/sector
    csc_sector_counter = np.zeros((2,2,6), dtype=np.int32) #endcap,station,sector
    #counting when muon present in iRPC eta range
    csc_sector_counter_muon = np.zeros((2,2,6), dtype=np.int32) #endcap,station,sector
    #counting_csc by station
    csc_station_counter = np.zeros((3,), dtype=np.int32)
    csc_station_counter_muon = np.zeros((3,), dtype=np.int32)

    def match_csc_hit(hit):
      match=True
      if hit.type == kCSC and ((hit.station == 3 or hit.station == 4) and hit.ring == 1):
        endcap=hit.endcap+1 if hit.endcap==-1 else hit.endcap
        station=hit.station-3
        sector=hit.sector-1
        csc_sector_counter[endcap,station,sector]+=1
        csc_station_counter[0]+=1
        csc_station_counter[hit.station-2]+=1
        #checking there is a muon in ME31 or ME41 eta coverage
        if abs(part.eta)>1.8 and abs(part.eta)<2.5:
          csc_sector_counter_muon[endcap,station,sector]+=1
          csc_station_counter_muon[0]+=1
          csc_station_counter_muon[hit.station-2]+=1
        #matching
        #if (hit.station==3 and hit.sim_eta>1.84) or (hit.station==4 and hit.sim_eta>1.92):
        #if (hit.station==3 and hit.sim_eta>1.82) or (hit.station==4 and hit.sim_eta>1.91):
        if (hit.station==3 and hit.sim_eta>1.85) or (hit.station==4 and hit.sim_eta>1.92):
          dx,dy,dR = self.nearestRPCtoCSC(hit,irpc_hits)
          self.histos.histos['CSC_iRPC_dx'].Fill(dx)
          self.histos.histos['CSC_iRPC_dy'].Fill(dy)
          self.histos.histos['CSC_iRPC_dxy'].Fill(dx,dy)
          self.histos.histos['CSC_iRPC_dR'].Fill(dR)
          if dR>cut_match_dR:
            self.histos.histos['CSC_no_matched'].Fill(1)
            match=False
          else:
            self.histos.histos['CSC_matched'].Fill(1)
      return match

    if cut_match_dR!=0:
      legit_hits = list(filter(match_csc_hit, legit_hits))
    #Fillig histos for event
    for endcap in [0,1]:
      for station in [0,1]:
        for sector in [0,1,2,3,4,5]:
          self.histos.histos_hits[endcap,station,sector].Fill \
          (csc_sector_counter[endcap,station,sector])

          self.histos.histos_hits_muon[endcap,station,sector].Fill \
          (csc_sector_counter_muon[endcap,station,sector])

    for i in [0,1,2]:
      self.histos.histos_station[i].Fill \
      (csc_station_counter[i])
      self.histos.histos_station_muon[i].Fill \
      (csc_station_counter_muon[i])
...
class RatesAnalysis(object):
  def run(self, omtf_input=False, run2_input=False, pileup=200, match_all_endcap=False, cut_match_dR=100):
    # Book histograms
    histograms = {}
    hname = "nevents"
    ...
    # Load tree
    tree = load_minbias_batch(jobid, file_first, file_last, pileup=pileup)
    ...
    for ievt, evt in enumerate(tree):
      ...
      # EMTF mode
      roads = recog1.run(evt.hits, part, cut_match_dR)
      clean_roads = clean.run(roads)
      slim_roads = slim.run(clean_roads)
      ...
      # OMTF mode
      roads2 = recog2.run(evt.hits, part)
    ...
    if use_condor:
      if match_all_endcap==True:
        outfile = 'histos_tbb_files_%i-%i_CSCmatch_EMTF_cutDR_%i.root' % (jobid,file_last, cut_match_dR)
      else:
        outfile = 'histos_tbb_files_%i-%i_CSCmatch_iRPC_cutDR_%i_v2.root' % (file_first,file_last, cut_match_dR)
...
class EffieAnalysis(object):
  def run(self, omtf_input=False, run2_input=False, match_all_endcap=False, cut_match_dR=100):
    # Book histograms
    histograms = {}
    eff_pt_bins = (0., 0.5, 1., 1.5, 2., 3., 4., 5., 6., 7., 8., 10., 12., 14., 16., 18., 20., 22., 24., 27., 30., 34., 40., 48., 60., 80., 120.)
    ...
    if omtf_input:
      ...
      tree = load_pgun_batch(jobid,file_first,file_last)
    ...
    if use_condor:
      if match_all_endcap==True:
        outfile = 'histos_tbc_files_%i-%i_CSCmatch_EMTF_cutDR_%i.root' % (file_first,file_last, cut_match_dR)
        # Save histograms Matching
        recog1.histos.write(outfile.replace('tbc','matching'))
      else:
        outfile = 'histos_tbc_files_%i-%i_CSCmatch_iRPC_cutDR_%i_v2.root' % (file_first,file_last, cut_match_dR)
        # Save histograms Matching
        recog1.histos.write(outfile.replace('tbc','matching'))
...
jobid = 0
if use_condor:
  #jobid = int(sys.argv[3].split('-')[0])
  #file_first=jobid
  file_first=int(sys.argv[3])
  file_last=int(sys.argv[4])
  #file_last= int(sys.argv[3].split('-')[1])

cut_match_dR=100
if use_condor and len(sys.argv)>5:
  cut_match_dR=int(sys.argv[5])

match_all_endcap=False
if use_condor and len(sys.argv)>6:
  if sys.argv[6]=='yes':
    match_all_endcap=True
...
def load_pgun_batch(j,file_first=1,file_last=2):
  #global infile_r
  #infile_r = root_open('pippo.root', 'w')
  infiles = []
  for i in range(file_first,file_last+1):
    #infiles.append('file:ntuple_SingleMuon_Endcap_%i.root' % i)
    infiles.append('file:data/ntuple_SingleMuon_Endcap_%i.root' % i)
    #infiles.append('root://cmsxrootd-site.fnal.gov//store/group/l1upgrades/L1MuonTrigger/P2_10_4_0/ntuple_SingleMuon_Endcap_2GeV/ParticleGuns/CRAB3/190416_194707/0000/ntuple_SingleMuon_Endcap_%i.root' % i)
...
def load_minbias_batch(j, file_first=1, file_last=1, pileup=200):
  #global infile_r
  pufiles = []
  if pileup == 140:
    ...
    for i in range(file_first,file_last+1):
      #pufiles.append('file:data/ntuple_SingleNeutrino_PU200_%i.root' % i)
      pufiles.append('file:data/ntuple_SingleMuon_Endcap_%i.root' % i)
  elif pileup == 250:
  ...
  print('[INFO] Opening file: %s' % ' '.join(pufiles))

  print('[INFO] Opening file: %s' % pufiles)
...
if __name__ == "__main__":
  ...
  if analysis == 'dummy':
    ...
    analysis.run(omtf_input=omtf_input, run2_input=run2_input, pileup=200, match_all_endcap=match_all_endcap, cut_match_dR=cut_match_dR)
    ...
    analysis.run(omtf_input=omtf_input, run2_input=run2_input, match_all_endcap=match_all_endcap, cut_match_dR=cut_match_dR)
